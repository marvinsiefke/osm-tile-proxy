...
- `$allowedReferers` An array of domains that are allowed as referrers. The referer can be optionally limited to a specific `hostname` (of the proxy) or a specific area `maxBounds`. Default: empty (all referrers allowed).
